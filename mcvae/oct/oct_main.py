...
from oct_ml_ae import AE_OCT
from pl_datamodule_oct import OCT_DM
from load_config import load_config
from oct_ml_module_ae import AE
...
#        #TOFIX: make this more general
    dm = OCT_DM(config.dir_imgs_ae, config.ids_set, img_size=config.input_dim[-3:], batch_size=config.optimizer.batch_size) 
    return dm

def ml_model1_trainer(config):
     dm_ae = get_datamodule(config)
     ae_oct_args = get_eye_args(config)
     ae_oct = AE_OCT(**ae_oct_args)
     model1 = AE(ae_oct, config)
     trainer_ae = pl.Trainer(accelerator="gpu", devices=1, callbacks=[EarlyStopping(monitor="avg_total_loss_validation", mode="min", patience=20)], max_epochs=-1) #, min_epochs=1)7
