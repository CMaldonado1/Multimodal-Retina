...
from retina_ml_vae import MCVAE
...
def ml_model_trainer(config):
     ...
     vae_fundus_args, vae_oct_args = get_eye_args(config)
     oct_vae = VAE_oct(**vae_oct_args)
     fundus_vae = VAE_FUNDUS(**vae_fundus_args)

     oct_weights = load_weight(config, 'oct')
     oct_vae.load_state_dict(oct_weights) #, strict=False)
    
    # Load pretrained weights for Fundus VAE
     fundus_weights = load_weight(config, 'fundus')
     fundus_vae.load_state_dict(fundus_weights) #, strict=False)

     _mcvae = MCVAE(fundus_vae, oct_vae)

     model2 =  mcvae(_mcvae, config)
