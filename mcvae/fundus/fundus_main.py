...
from oct_ml_vae_fc2 import VAE_FUNDUS
from fundus_ml_ae import AE_fundus
from pl_datamodule_fundus import fundus_DM
from load_config import load_config
from fundus_ml_module_ae import AE
...
#        #TOFIX: make this more general
    dm = fundus_DM(config.dir_imgs_ae, config.ids_set, img_size=config.input_dim[-3:], batch_size=config.optimizer.batch_size) 
    return dm
...
def ml_model2_trainer(config):
     ...
     vae_fundus_args = get_eye_args(config)
     fundus_vae = VAE_fundus(**vae_fundus_args)
     ckpt_= torch.load(config.pretrained_model, map_location= torch.device('cuda:0'))
     _model_pretrained_weights = {k.replace("model.", ""): v for k, v in ckpt['state_dict'].items()} #if "fc"  in k}
     model2 =  VAE(fundus_vae, config)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='AE fundus')
    parser.add_argument('--config', default = 'config_fundus.yaml')
