...
from fundus_ml_vae_trans import VAE_FUNDUS, vae_classifier_fundus, classifier_FUNDUS
from fundus_ml_ae import AE_fundus
from pl_datamodule_fundus import fundus_DM
from load_config import load_config
from fundus_ml_module_class import AE
...
#        #TOFIX: make this more general
    dm = fundus_DM(config.dir_imgs_ae, config.ids_set_class, img_size=config.input_dim[-3:], batch_size=config.optimizer.batch_size) 
    return dm
...
def ml_model2_trainer(config):
     ...
     vae_fundus_args, class_fundus_args = get_eye_args(config)
